...
$scope.Job={jobprofile:'',jobdesc:'',qualification:'',salary:'',company:'',companydesc:''};	
	...
	function fetchAllJobs()
	{
		...
		$http.get("http://localhost:8080/colloboration_middleware/jobs/getAllJobs")
		...
		$http.get("http://localhost:8080/colloboration_middleware/jobs/myjobs/"+$rootScope.currentuser.user_id)
	...
	$scope.insertJobs = function()
	{
		...
		$http.post('http://localhost:8080/colloboration_middleware/jobs/addJob',
	...
	$scope.applyJob = function(idd)
	{
		...
		$http.get('http://localhost:8080/colloboration_middleware/jobs/applyJob/'+idd+"/"+$rootScope.currentuser.user_id).then(fetchAllJobs(), function(response) {
	...
	$scope.getjob = function(idd)
	{
		...
		$http.get('http://localhost:8080/colloboration_middleware/jobs/getJob/'+idd).then(function(response) {
		...
		$http.get('http://localhost:8080/colloboration_middleware/jobs/checkifapplied/'+idd+"/"+$rootScope.currentuser.user_id).then(function(response) {
		...
		$http.get("http://localhost:8080/colloboration_middleware/jobs/jobapplicants/"+idd)
		.then(function(response)
		{
	...
	$rootScope.deletejob = function(idd)
	{
		$http({
	        ...
	        url: 'http://localhost:8080/colloboration_middleware/jobs/deleteJob/'+idd,
	        transformResponse: [
	            function (data) { 
	                return data; 
	            }
...
/*	$scope.editjob=function(idd)
	{
		...
		$http.get('http://localhost:8080/colloboration_middleware/jobs/updateJob/'+idd).then(function(response) {
	...
	if($scope.Job.jobprofile==null)
		...
		$http.post("http://localhost:8080/colloboration_middleware/jobs/updateJob/",$scope.Job).then(fetchAllJobs(), function(response) {
